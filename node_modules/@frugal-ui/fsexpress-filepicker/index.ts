...
import { mkdir, readdir, rename, rm } from 'basic-fs-express-frontend';
...
export default function FilePicker(rootName: string, currentPath: BindableObject<string>, allowedTypes: Types[]) {
...
class DirectoryScene extends GenericScene<DirectorySceneCfg> {
	draw(data: DirectorySceneCfg) {
		...
		const isNewDirPopoverOpen = new State(false);
		const newDirectoryName = new State('');
		async function createDirectory() {
			const name = newDirectoryName.value;
			try {
				await mkdir(`${data.path}/${name}`);
			} catch {
				alert(`Failed to create directory "${name}"`);
			}

			isNewDirPopoverOpen.value = false;
			updateList();
		}
		...
		return VStack(
			Header(
				...
				Popover({
					isOpen: isNewDirPopoverOpen,
					accessibilityLabel: 'create directory',
					toggle: Button({
						iconName: 'create_new_folder',
						accessibilityLabel: 'create directory',
						action: () =>
							(isNewDirPopoverOpen.value =
								!isNewDirPopoverOpen.value),
					}),
					content: VStack(
						Label(
							'Name',
							Input(
								new TextInputCfg(newDirectoryName, 'Untitled'),
							),
						),
						Button({
			...
			DirectoryItemList(data.path, this, listUpdater, data.currentPath, data.allowedTypes),
...
function DirectoryItemList(
	...
	async function refresh() {
		...
		for (let i = 0; i < listing.length; i++) {
			...
			const matchInItemMap = itemMap.values().find(mapItem => mapItem.name == item.name);
		...
		//remove items
		itemMap.forEach((item) => {
			const matchInListing = listing.find(listingItem => listingItem.name == item.name);
...
function DirectoryItemView(configuration: DirectoryItemViewCfg) {
	...
	return component
		.listen('click', () => {
			if (item.isDirectory == true && configuration.allowedTypes.indexOf(Types.Directory) == -1) return;
			if (item.isDirectory == false && configuration.allowedTypes.indexOf(Types.File) == -1) return;
...
function ItemText(configuration: ItemTextCfg, iconName: string) {
	return [
...
function ItemOptionsMenu(configuration: ItemTextCfg) {
	const { path, listUpdater, parent } = configuration;
